...
var products = pgTable("products", {
  ...
  sizes: varchar("sizes").array().default(sql`'{}'::varchar[]`),
...
var DatabaseStorage = class {
  ...
  async getProducts(categorySlug) {
    if (categorySlug) {
      return await this.getProductsByCategory(categorySlug);
    }
  ...
  async addToCart(cartItem) {
    ...
    if (existing) {
      const [updated] = await db.update(cartItems).set({ quantity: (existing.quantity || 0) + (cartItem.quantity || 1) }).where(eq(cartItems.id, existing.id)).returning();
      return updated;
    } else {
  ...
  async getOrders(userId) {
    const baseQuery = db.select().from(orders).leftJoin(orderItems, eq(orders.id, orderItems.orderId)).leftJoin(products, eq(orderItems.productId, products.id)).orderBy(desc(orders.createdAt));
    const query = userId ? baseQuery.where(eq(orders.userId, userId)) : baseQuery;
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/products", async (req, res) => {
    try {
      ...
      const products2 = await storage.getProducts(categorySlug);
      res.json(products2);
    } catch (error) {
  ...
  app2.get("/api/products/category/:slug", async (req, res) => {
    try {
      const products2 = await storage.getProductsByCategory(req.params.slug);
      res.json(products2);
    } catch (error) {
  ...
  app2.post("/api/orders", isAuthenticated, async (req, res) => {
    try {
      ...
      for (const item of items) {
        ...
        total += parseFloat(product.salePrice || product.price) * item.quantity;
      }
      const orderData = {
        userId,
      ...
      const orderItems2 = items.map((item) => ({
        ...
        price: item.price,
        orderId: ""
        // This will be set in storage.createOrder
      }));
